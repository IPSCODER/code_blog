...
const Header = () => {
  ...
  return (
    <header className='w-full p-4 px-5 sm:px-10 flex items-center justify-between' >
        <Logo/>
        <button className='inline-block sm:hidden z-50' onClick={() => setClick(prev => !prev)}>
        ...
        <nav className='w-max py-3 px-8 border border-solid border-dark rounded-full font-medium capitalize flex items-center fixed top-6 right-[50%] translate-x-[50%] bg-light/80 backdrop-blur-sm z-50 -translate-y-[200%] sm:translate-y-0 transition-all ease duration-200' style={{transform:click && "translateY(0) translateX(50%) "}} >
        ...
        <div className='hidden sm:flex items-center' >
            ...
            <a href={siteMetadata.github} className='inline-block w-6 h-6 mr-4' ><GithubIcon className={"hover:scale-125 transition-all dark:fill-light ease duration-200"} /></a>
